...
def main():
    ...
    for img_path in test_images:
        try:
            ...
            # YOLO method
            print("Method 1: YOLO + TinySAM (Ours)")
            yolo_result = evaluate_yolo_method(img_path, yolo_model, predictor)
            yolo_results.append(yolo_result)
            print(f"  Time: {yolo_result['total_time']*1000:.1f}ms | FPS: {yolo_result['fps']:.2f} | Masks: {yolo_result['num_masks']} | IoU: {yolo_result['mean_iou']:.4f}")
            
            # Hierarchical method
            print("Method 2: Hierarchical Everything (Original)")
            hier_result = evaluate_hierarchical_method(img_path, mask_generator)
            hierarchical_results.append(hier_result)
            print(f"  Time: {hier_result['total_time']*1000:.1f}ms | FPS: {hier_result['fps']:.2f} | Masks: {hier_result['num_masks']} | IoU: {hier_result['mean_iou']:.4f}")
    ...
    yolo_fps_mean = np.mean([r['fps'] for r in yolo_results])
    yolo_iou_mean = np.mean([r['mean_iou'] for r in yolo_results])
    yolo_time_mean = np.mean([r['total_time'] for r in yolo_results]) * 1000
    
    hier_fps_mean = np.mean([r['fps'] for r in hierarchical_results if r['fps'] > 0])
    hier_iou_mean = np.mean([r['mean_iou'] for r in hierarchical_results if r['mean_iou'] > 0])
    hier_time_mean = np.mean([r['total_time'] for r in hierarchical_results if r['total_time'] > 0]) * 1000
    
    print(f"\n{'Method':<40} {'FPS':<10} {'Time(ms)':<12} {'Mean IoU':<10} {'#Masks'}")
    print("-"*80)
    print(f"{'YOLO + TinySAM (Ours)':<40} {yolo_fps_mean:<10.2f} {yolo_time_mean:<12.1f} {yolo_iou_mean:<10.4f} {np.mean([r['num_masks'] for r in yolo_results]):.1f}")
    print(f"{'Hierarchical Everything':<40} {hier_fps_mean:<10.2f} {hier_time_mean:<12.1f} {hier_iou_mean:<10.4f} {np.mean([r['num_masks'] for r in hierarchical_results]):.1f}")
    print("-"*80)
    ...
    if hier_time_mean > 0:
        ...
        print(f"  - Quality: {iou_change:+.1f}% IoU change")
        print(f"  - Efficiency: {speedup*yolo_iou_mean/hier_iou_mean:.2f}x better (speed × quality)")
